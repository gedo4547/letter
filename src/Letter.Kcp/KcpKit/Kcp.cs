...
public partial class Kcp
{
  ...
  private int incr;
  private  List<KcpSegment> snd_queue = new List<KcpSegment>();
  private  List<KcpSegment> rcv_queue = new List<KcpSegment>();
  private  List<KcpSegment> snd_buf = new List<KcpSegment>();
  private  List<KcpSegment> rcv_buf = new List<KcpSegment>();
   ...
   * @param user
   */
  public Kcp(int conv, IOutput output, Object user)
  {
    this.conv = conv;
  ...
  public int PeekSize()
  {
    ...
    KcpSegment seq = rcv_queue.First();
    if (seq.frg == 0)
    {
  ...
  private void Parse_data(KcpSegment newseg)
  {
    int sn = newseg.sn;
      ...
      switch (cmd)
      {
        ...
        case IKCP_CMD_PUSH:
          if (_itimediff(sn, rcv_nxt + rcv_wnd) < 0)
          {
              ...
              {
                  KcpSegment seg = new KcpSegment(len);
                  seg.conv = conv_;
                  seg.cmd = cmd;
  ...
  private void Flush()
  {
    ...
    KcpSegment seg = new KcpSegment(0);
    seg.conv = conv;
    ...
    {
      KcpSegment item=snd_queue[i];
      if (_itimediff(snd_nxt, snd_una + cwnd_temp) >= 0)
      {
        break;
      }
      KcpSegment newseg = item;
      newseg.conv = conv;
